...
mod map_3d {
    ...
    pub fn wgs84() -> (f64,f64,f64,f64) {
        ...
        let ecc = ((major*major)-(minor*minor))/(major*major);
        
        (major,minor,flattening,ecc.sqrt())
    }
    pub fn get_radius_normal(lat: f64)->f64 {
        let (major,_,_,_) = wgs84();
        let sq_major = major*major;
        let den = (sq_major*lat.cos()*lat.cos()+sq_major*lat.sin()*lat.sin()).sqrt();
